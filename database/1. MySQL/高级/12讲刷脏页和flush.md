...
在前面第2篇文章[《日志系统：一条SQL更新语句是如何执行的？》](https://time.geekbang.org/column/article/68633)中，我为你介绍了WAL机制。现在你知道了，InnoDB在处理更新语句的时候，只做了写日志这一个磁盘操作。这个日志叫作redo log（重做日志），也就是《孔乙己》里咸亨酒店掌柜用来记账的粉板，在**更新内存写完redo log**后，就返回给客户端，本次更新成功。
...
![img](img/349cfab9e4f5d2a75e07b2132a301fda.jpeg)
...
- 第一种场景是，粉板满了，记不下了。这时候如果再有人来赊账，掌柜就只得放下手里的活儿，将粉板上的记录擦掉一些，留出空位以便继续记账。当然在擦掉之前，他必须先将正确的账目记录到账本中才行。
  这个场景，对应的就**是InnoDB的redo log写满**了。这时候系统会停止所有更新操作，把checkpoint往前推进，redo log留出空间可以继续写。我在第二讲画了一个redo log的示意图，这里我改成环形，便于大家理解。

  ![img](img/a25bdbbfc2cfc5d5e20690547fe7f2e5.jpg)
...
![img](img/cc44c1d080141aa50df6a91067475374.png)

图3 InnoDB刷脏页速度策略

> 缓存中脏页比例越多，那么提高速度刷脏页，空出内存；如果 redo log空间越来越少，那么加快。并且这两者应该有关联，flush脏页也会刷掉部分redo log的数据？内部逻辑。
...
![img](img/ce7f4e35916ed1aa49206a53a0547bd9-1579747097084.jpg)
