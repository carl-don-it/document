...
> - 泛型接口、类、方法定义上的所有泛型、**成员变量**声明处的泛型都会被保留类型信息，其他地方的泛型信息都会被擦除。
> - localvariabletypetable可以获取变量声明的泛型，但不是变量实际的泛型，被擦除了，而且这种方法太麻烦了，可能需要回溯调用栈。
...
可以给**类型形参**限制一个边界也可以限制多个边界，在限制多个边界的时候我们要注意的是：
